...
async def start_stream(room_id: str, stream_data: StreamRequest, current_user: User = Depends(get_current_user)):
    ...
    if "youtube.com/watch" in stream_url or "youtu.be" in stream_url or "youtube.com/live" in stream_url:
        ...
        if video_id:
            # mute=1 required for autoplay on iOS, controls=0 to hide YouTube UI
            embed_url = f"https://www.youtube-nocookie.com/embed/{video_id}?autoplay=1&mute=1&modestbranding=1&rel=0&controls=0&showinfo=0&vq=hd1080&playsinline=1"
