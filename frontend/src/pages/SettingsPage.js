...
import { 
  ...
  Heart, UserPlus, Megaphone, Check, X, Camera, AtSign, Image, Users, UserMinus
} from 'lucide-react';
...
const SettingsPage = ({ user, onLogout }) => {
  ...
  const handleImageSelect = async (e) => {
    ...
    if (file.size > 2 * 1024 * 1024) {
      toast.error(isRTL ? 'حجم الصورة كبير جداً (الحد الأقصى 2MB)' : 'Image too large (max 2MB)');
      return;
    }
    ...
    try {
      ...
      toast.error(txt.imageError);
      setUploadingImage(false);
    }
  };

  const handleRandomAvatar = () => {
    const seeds = ['Felix', 'Aneka', 'Milo', 'Zoe', 'Max', 'Luna', 'Leo', 'Bella', 'Oscar', 'Coco', 'Nala', 'Simba'];
  ...
  const handleSaveProfile = async () => {
    ...
    try {
      const response = await axios.put(
        `${API}/auth/profile`,
        { username: profileData.username, bio: profileData.bio, avatar: profileData.avatar },
        { headers: { Authorization: `Bearer ${token}` } }
      );
    ...
    <button onClick={toggle ? () => onToggle(!value) : onClick}
      ...
      {toggle ? (
        <div className="flex items-center gap-3">
          ...
          <div className={`w-14 h-8 rounded-full flex items-center px-1 transition-all duration-300 ${value ? 'bg-lime-400' : 'bg-slate-700'}`}>
  ...
  const UserListItem = ({ u, showFollowBtn = true }) => (
    <div className={`flex items-center gap-3 p-4 hover:bg-slate-800/50 ${isRTL ? 'flex-row-reverse' : ''}`}>
      ...
      <div className={`flex-1 ${isRTL ? 'text-right' : 'text-left'}`}>
        <p className="text-white font-cairo font-bold">{u.username}</p>
        {u.bio && <p className="text-slate-500 text-sm font-almarai line-clamp-1">{u.bio}</p>}
      </div>
      {showFollowBtn && u.id !== user.id && (
  ...
  const MainView = () => (
    <>
      ...
      <button onClick={() => setShowLogoutConfirm(true)}
        className={`w-full p-4 flex items-center gap-4 hover:bg-red-500/10 ${isRTL ? 'flex-row-reverse' : ''}`}>
    ...
    <>
      <SubPageHeader title={isEditingProfile ? txt.editProfile : txt.profile} onBack={() => { setCurrentView('main'); setIsEditingProfile(false); }}
        rightButton={isEditingProfile ? (
          <button onClick={handleSaveProfile} disabled={savingProfile}
            className="px-5 py-2 rounded-full bg-lime-400 text-slate-900 font-cairo font-bold text-sm hover:bg-lime-300 disabled:opacity-50">
            {savingProfile ? '...' : txt.save}
          </button>
        ) : (
          <button onClick={() => setIsEditingProfile(true)}
            className="px-5 py-2 rounded-full border border-slate-600 text-white font-cairo font-bold text-sm hover:bg-slate-800">
            {txt.edit}
          </button>
        )}
      />
      ...
      <div className="relative">
        ...
        <div className="absolute -bottom-16 left-1/2 -translate-x-1/2">
          <div className="relative">
            <img src={isEditingProfile ? profileData.avatar : user.avatar} alt={user.username}
              className="w-32 h-32 rounded-full border-4 border-slate-950 bg-slate-900 object-cover" />
            {isEditingProfile && (
              <button onClick={() => fileInputRef.current?.click()}
                className="absolute bottom-2 right-2 w-10 h-10 rounded-full bg-lime-400 flex items-center justify-center shadow-lg hover:bg-lime-300">
        ...
        {isEditingProfile ? (
          <div className="space-y-4">
            ...
            <div className="flex justify-center gap-3 mb-4">
              <button onClick={() => fileInputRef.current?.click()}
                className="flex items-center gap-2 px-4 py-2 rounded-full bg-slate-800 text-white text-sm font-almarai hover:bg-slate-700">
            ...
            <div>
              ...
              <div className="relative">
                ...
                <Input value={profileData.username} onChange={(e) => setProfileData({ ...profileData, username: e.target.value })}
                  className={`bg-slate-800 border-slate-700 text-white ${isRTL ? 'pr-10 text-right' : 'pl-10'}`} maxLength={20} />
              </div>
            </div>
    ...
    <>
      ...
      {followers.length === 0 ? (
        <div className="p-8 text-center">
          ...
          <p className="text-slate-500 font-almarai">{txt.noFollowers}</p>
        </div>
      ) : (
  ...
  const FollowingView = () => (
    <>
      ...
      {following.length === 0 ? (
        <div className="p-8 text-center">
          <Users className="w-16 h-16 text-slate-700 mx-auto mb-4" />
          <p className="text-slate-500 font-almarai">{txt.noFollowing}</p>
        </div>
      ) : (
      ...
      <div className="divide-y divide-slate-800/50">
        <div className={`p-4 flex items-center justify-between ${isRTL ? 'flex-row-reverse' : ''}`}>
          ...
          <div className={`flex items-center gap-2 ${isRTL ? 'flex-row-reverse' : ''}`}>
            <Mail className="w-4 h-4 text-slate-500" /><p className="text-white font-cairo">{txt.email}</p>
          </div>
        </div>
  ...
  const DisplayView = () => (
    <>
      ...
      <div className="divide-y divide-slate-800/50">
        <SettingItem icon={settings.darkMode ? Moon : Sun} label={settings.darkMode ? txt.darkMode : txt.lightMode} desc={settings.darkMode ? txt.darkModeDesc : txt.lightModeDesc} toggle value={settings.darkMode} onToggle={(v) => updateSetting('darkMode', v)} status />
      ...
      <div className="divide-y divide-slate-800/50">
        {['ar', 'en'].map(lang => (
          <button key={lang} onClick={() => setLanguage(lang)}
            className={`w-full p-4 flex items-center justify-between hover:bg-slate-800/50 ${isRTL ? 'flex-row-reverse' : ''}`}>
            <div className={`flex items-center gap-3 ${isRTL ? 'flex-row-reverse' : ''}`}>
              <span className="text-2xl">{lang === 'ar' ? '🇸🇦' : '🇺🇸'}</span>
              <div className={isRTL ? 'text-right' : 'text-left'}>
                <p className="text-white font-cairo">{lang === 'ar' ? 'العربية' : 'English'}</p>
                <p className="text-slate-500 text-sm font-almarai">{lang === 'ar' ? 'Arabic' : 'الإنجليزية'}</p>
              </div>
            </div>
  ...
  const AboutView = () => (
    <>
      ...
      <div className="p-8 flex flex-col items-center">
        <motion.div className="w-24 h-24 bg-gradient-to-br from-lime-400 to-emerald-500 rounded-3xl flex items-center justify-center mb-4 shadow-lg" whileHover={{ scale: 1.05 }}>
    ...
    <div className="min-h-screen bg-slate-950">
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-slate-900/95 backdrop-blur-xl border-t border-slate-800 z-40">
        <div className={`max-w-[600px] mx-auto flex justify-around p-4 ${isRTL ? 'flex-row-reverse' : ''}`}>
          <button onClick={() => navigate('/dashboard')} className="flex flex-col items-center gap-1 text-slate-400 hover:text-white">
            <Home className="w-6 h-6" /><span className="text-xs font-almarai">{t('home')}</span>
          </button>
          <button onClick={() => navigate('/matches')} className="flex flex-col items-center gap-1 text-slate-400 hover:text-white">
            <Trophy className="w-6 h-6" /><span className="text-xs font-almarai">{t('matches')}</span>
          </button>
          <button className="flex flex-col items-center gap-1 text-lime-400">
            <Settings className="w-6 h-6" /><span className="text-xs font-almarai">{t('settings')}</span>
          </button>
        </div>
      </div>
    </div>
